...
public class DiabeticRecipesTest {
	...
	public void recipeScrappingSouthIndian() throws Exception {
		...
		ExcelUtils xlUtils= new ExcelUtils("files/Diabetic.xlsx");
		...
		xlUtils.writeToExcel(data,"DiabeticRecipes",0);
		...
		for(int i=0;i<links.size();i++)//links.size()
		{
			...
			data.add(imageLink);
			data.add(recipeLink);
			xlUtils.writeToExcel(data,"DiabeticRecipes",i);
